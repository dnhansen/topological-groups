...
\end{equation*}
%
We show below that $\tilde{\rho}$ is a pseudo-metric, and we call it the \emph{quotient metric} on $G/H$.
...
\begin{proposition}[Quotient metric]
    ...
    The map $\tilde{\rho}$ is a pseudo-metric on $G/H$ compatible with the quotient topology. If $H$ is normal such that $G/H$ is a group, then $\tilde{\rho}$ is invariant. Furthermore, $\tilde{\rho}$ is a metric if and only if $H$ is closed.
\end{proposition}
...
\begin{proof}
    ...
    \end{equation*}
    %
    Fix $x \in G$ and notice that $\set{B_\rho(x,r)}{r > 0}$ is a local basis at $x$. Since $q(B_\rho(x,r)) = B_{\tilde{\rho}}(q(x),r)$ by the argument above, the set $\set{B_{\tilde{\rho}}(q(x),r)}{r > 0}$ is a local basis at $q(x)$. Hence $\tilde{\rho}$ is compatible with the topology on $G/H$.
